...
author: "袁慎建"
brief: "
如今程序员赶上了中国最庞大的群体`农民`，大街上抓一把，十个中有八个是程序员，剩下的一个是项目经理，另一个则刚从某国企离职报名参加软件培训班。我想`码农`的称号或许这么来着吧。
</br></br>在外行人看来，程序员是一个成天对着电脑倒腾着代码、看着Terminal上行云流水般的打印、过着不修边幅的日子外加超负荷的码农。
</br></br>在内行人看来，程序员是一个成天面对QA的'质疑'、PM的'夺命催'以及DEVs的'吐槽'，扛着身心压力`苦行曾`。
"
...
在内行人看来，程序员是一个成天面对QA的"质疑"、PM的"夺命催"以及DEVs的"吐槽"，扛着身心压力的`苦行曾`。
...
首先，我们可以摆出苦行曾的心态，平日里练就一身好把式：如Clean Code、Refactor、OOD及FOP。即便这样，牛逼哄哄的程序员也不敢说自己的代码百分之百没有缺陷。

怎么办，两个参考原则：

1. 编写完代码多问自己一句："真的可靠地完成目标了吗？" 怎么问，写个测试来提问题。这便是 **测试覆盖**。
2. 编写代码之前先问自己一句："怎么样才算完成目标了呢？" 怎么问，同样写个测试来提问。这便是 **TDD + 测试覆盖**。

![图片来自网络]({{ site.url }}{{ site.img_path }}{{ '/tdd/ku-xing-ceng.png' }})
...
>广义上的测试并不从事像上面这段代码这么简单，熟为人知的 [测试金字塔](https://martinfowler.com/bliki/TestPyramid.html) 将测试分为三大类，单元测试位于测试金字塔底端，旨在传达单元测试总是应该来得更凶猛一些，而它们也是由开发人员亲手编写出来。本文也是围绕单元测试来开展。
...
经常听开发人员说："我对我的代码非常有信心。"理由往往很充分并单一：*单元测试是老大，老大罩着我不怕*。（当然，专业的QA始终能发现DEV很难察觉的Defect，难免会惊起一脸狐疑：*老大不灵了吗！*。回首代码，觉漏某一Case）。
...
所以被这种声音迷惑的人开始信奉了*长（测）话（试）短（少）说（写），短（甚）话（至）不（不）说（写）*的信仰。这经过修饰得以传播的一种声音，而背后做支撑的总有那么几大派系。
...
实践证明，随着时间推移，产品的性能的变化趋势受测试代码编写的时机的影响如下图所示：
...
> 就我个人经验，半TDD的编码方式，在一个Story上所花的总时间不会多余没有测试裸奔的代码。或许刚开始会觉得有点拖慢节奏，操练多了，它的威力就会彰显出来。

测试也写了，可是运行时间太长了又带来了苦恼？

细谈该苦恼可以单独写一篇文章了。我的确见过测试运行时间很长，每次验证一次跑上半个多小时。下面列举一些加快测试的运行时间的实践：
...
单元测试运行时间是毫秒级别的，如果耗时过长，你就要留意否存在内存泄漏、资源未释放、依赖过重或者不依赖容器而启动了容器的单元测试。

---

## 挥之不去的例外
编写单元测试是一项成本低却价值很高的活动。编写它不会花掉你太多的时间，而运行它更是毫秒间的事情。极限编程推崇者正在使用TDD的方式正在诠释单元测试的价值的意义。

它能带给我们信心，改良我们的代码的设计，提升我们（DEVs）的声誉，为代码库保驾护航，为高质量的软件交付提供保障。但它终究不是一颗银弹。我们*编写单元测试也无非一种价值的取舍*，当它给我们带来的价值低于我们付出的成本时，我们就要保持警惕了，比如思考以下两个问题：
